...
| ----------------------------------------------------- | ---------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| Layers                       | 1 - 6                        | Number of feed forward layers to include where 2 layers include 32 and then 8 nodes while 3 layers include 64, 32, and 8. Layer sizes are {256, 128, 64, 32, 16, 8}. | More layers might allow networks to learn more sophisticated behaviors but also might overfit to input data.                         |
...
However, while these SSP245 yield means remain similar to the historic baseline, Figure @fig:hist reveals how the distribution tails differ more substantially, showing a doubling in counterfactual loss probability (2050 series vs counterfactual).
...
Prior work suggests that historic trends would anticipate continued increases in maize outputs [@nielsen_historical_2023] but our simulations predict climate change to substantially reduce the {{ counterfactualMean2050 }} yield increase that it would otherwise expect within the counterfactual simulation. In the place of that growth, we find yields under SSP245 to slightly drop in 2050 relative to current levels. Notably, this decrease in average yields impacts the simulated claims rate which goes to {{ experimentalProbability2030 }} at 2030 and {{ experimentalProbability2050 }} at 2050. This evolving profile arises in part due to the non-linear impacts of climate change where the yield expectation ($y_{expected}$) against which rates are set^[This decrease may impact not just aggregate output but also grower usage of the trend adjustment option when reporting their yield expectations [@plastina_trend-adjusted_2014].] may be lower in 2050 than 2030 but climate change still progresses further under SSP245.

## Risk unit size
Prior work expects larger insured units to reduce risk [@knight_developing_2010] and we similarly observe a claims rate decrease as the acreage included in an insured unit grows. Still, after attempting multiple insured unit sizes including removal of smaller Optional Units [@zulauf_importance_2023] in post-hoc simulations, a gap persists in claims rates between the counterfactual and SSP245, suggesting our concerns may remain relevant across different risk units.

## Geographic bias
Neighborhoods with significant results ($p < 0.05 / n$) may be more common in some areas as shown in Figure @fig:geo.

![Interactive geographic view. Color describes type of change and larger dots are larger areas of maize growing activity. Iowa, Illinois, and Indiana see possible concentrations of increased risk.](./img/map.png "Interactive geographic view. Color describes type of change and larger dots are larger areas of maize growing activity. Iowa, Illinois, and Indiana see possible concentrations of increased risk."){ width=85% #fig:geo }
...
Even so, as seen in Figure @fig:chirps, we caution that analysis finds $\rho = -0.2$, a weaker rank correlation indicating that model expectations cannot be described by temperature and precipitation alone.
