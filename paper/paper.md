...
We predict mean and standard deviation of yield deltas per neighborhood per year ahead of Monte Carlo simulations. This use of summary statistics helps ensure appropriate dimensionality for the dataset size given approximate normalilty in most neighborhoods [@kim_statistical_2013]. See supplemental for further statistics and interactive tools for further exploration. To predict these two responses, we describe each of the 9 CHC-CMIP6 variables as min, max, mean, count, and standard deviation per month for the given year. Along with the historic absolute yield mean and standard deviation seen in the neighborhood, these varibles constitute the input vector.
