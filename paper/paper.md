...
## Unit optimization
Field-level yield data and the actual geographically specific risk unit structure are not currently public so we sample units randomly based on expected size. However, as further explored in supplemental, growers likely optimize their own unit structure when purchasing policies, likely causing an under-estimation of claims rates. That in mind, the relative change in claims rate within our simulations is more salient: the doubling from current to 2050 is more important than the claims rates themselves as unit structure optimization would be active in both. Similarly, we do not have the geographically specific data required to model policy mechanisms of trend adjustment [@plastina_trend-adjusted_2014] and yield exclusion years [@schnitkey_yield_2015]. However, as both likely raise $y_{expected}$, excluding these two factors likely leads to a supression of loss rates in our simulations especially for SSP245. Altogether, recent actual claims rates which account for these factors are about 14% [@rma_statecountycrop_2024]. 
