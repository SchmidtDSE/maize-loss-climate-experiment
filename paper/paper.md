...
## Unit structure
Field-level yield data and the actual risk unit structure are not currently public so we sample units randomly based on expected size. However, as further explored in supplemental, growers likely optimize their own unit structure when purchasing policies, causing an under-estimation of claims rates. The 2016 actual claims rate is about 14% for the population considered [@rma_statecountycrop_2024]. That in mind, the change in claims rate within our simulations is more salient: the doubling from current to 2050 is more important than the claims rates themselves as unit structure optimization would be active in both. 
