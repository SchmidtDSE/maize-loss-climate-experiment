...
## Unit structure
Field-level yield data and the actual risk unit structure are not currently public so we sample units randomly based on expected size. However, as further explored in supplemental, growers likely optimize their own unit structure year to year. This likely causes an under-estimation of claims rates. Using 2016 as example, the actual claims rate is about 14% for the population considered [@rma_statecountycrop_2024]. That in mind, we suggest that the change in claims rate within our simulations is more salient than the claims rates themselves as this unit structure optimization will likely continue into the future.
