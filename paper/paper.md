...
These public interactive visualizations allow for further exploration of our modeling such as different loss thresholds for other insurance products, finding relationships of outcomes to different climate variables, answering geographically specific questions beyond the scope of this study, and modification of machine learning parameters to understand performance. In preparation for this open science release, we also report^[We collect information about the tool only and not generalizable knowledge about users or these patterns, falling under "quality assurance" activity. IRB questionnaire on file.] briefly on design changes in resposne to its participation in a 9 person "real-world" workshop session co-exploring these results similar to @pottinger_combining_2023.

 - Facilitators report simple single loop [@brazie_designing_2024] designs performing best within the limited time of the workshop but the two loop neighborhood simulator requires too long, slowing down the experience. In response, we frame that tool as supplemental in the public release of the software.
 - As expected by the design for juxtaposition gratification [@jm8_secret_2024], facilitators observe workshop discussion contrasting different results sets and configurations of models but, as the meta-parameter visualization in particular requires many interactions where the "best configuration" may be difficult to remember, we acted on suggestion to include a "sweep" button to see all results at once, as reflected in that tool's public release.

While not controlled experimentation [@lewis_using_1982] or diary studies [@shneiderman_strategies_2006] that could create more generalizable design knwoledge, these facilitator insights separately offer valuable qualitative insights informing real-world use of this explorable explanation [@victor_explorable_2011].
