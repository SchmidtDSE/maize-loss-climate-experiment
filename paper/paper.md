...
### Under-estimation
Despite this similarity between predictions and the comparable recent actuals, a number of difficult to model factors likely lead us to underestimate the actual claims rate in practice. First, field-level yield data and the actual geographically specific risk unit structure are not currently public. Therefore, while we sample units randomly based on expected size, growers likely optimize their own unit structure when purchasing policies to optimize financial upside as further explored in supplemental. Similarly, we do not have the geographically specific data required to model trend adjustment and yield exclusion options^[These increase $y_{expected}$ to be higher than the historic average[@plastina_trend-adjusted_2014] or remove particularly poor years from the average used to calculate $y_{expected}$ under certain conditions [@schnitkey_yield_2015].]. These both likely increase^[Historically-consistent expected increases in yields [@plastina_trend-adjusted_2014] outside our model likely partially negate trend adjustment. For this mechanism, financial incentives for growers may lead $y_{expected}$ to offset changes in claims rates caused by yield improvements.] the actual claims rates by raising $y_{expected}$.
