...
# Methods
We first build predictive models of maize yield distributions using a neural network at a spatial scale relevant to insurers. We then estimate changes to yield losses under different climate conditions with Monte Carlo simulation in order to estimate crop loss probability and severity. These changes to yield made relative to immediately prior averages at the same locations can then estimate expected insurance claims rates.
...
### Neural network
Our regressors ($f$) use neighborhood-level climate variables ($C$), year ($x$), and historic yield mean / std to predict future neighborhood-level mean and standard deviation of yield changes ($y_{\Delta\%}$) per year [@kim_investigating_2024]. We preprocess these inputs using z score normalization.

$y_{\Delta\%}(x) = \frac{y_{actual} - y_{expected}}{y_{expected}} = \frac{y_{\Delta}}{y_{\mu-historic}} = f(C_z, x, z_{\mu-historic}, z_{\sigma-historic})$
...
## Claims rate rebound
The claims rate is higher in the 2030 series compared to the 2050 series for our SSP245 simulations. To understand this, consider that yields generally increase over time [@nielsen_historical_2023] but our simulations anticipate more yield gains between 2030 and 2050 ({% if counterfactualMean2050|float > 0 %}+{% endif %}{{counterfactualMean2050}}) compared to historic and 2030 ({% if counterfactualMean2030|float > 0 %}+{% endif %}{{counterfactualMean2030}}) within SSP245. This may reflect that the change in temperature for SSP245 within CHC-CMIP6 [@williams_high_2024] is more extreme from the historic series to 2030 than it is for 2030 series to 2050. For example, using average daily maximum temperatures, the change from 2030 to 2050 is about one third (37%) lower than the change from historic to 2030. In other words, this "slow down" may allow historically consistent increases in yields to partially compensate for worsening growing conditions.

### Alternative simultions
This "rebound" is consistent across multiple executions of the pipeline and different sets of future years sampled. However, if simulations supress these mean yield changes over time that are consistent with historic trends, 2050 sees slightly higher claims rates than 2030. This reflects worsening variability over time (average standard deviation of % in 2030 series versus % in 2050 series) even if neighborhood means hold constant or improve.

### Institutional implications
Altogether, this observation may serve as an example of an important reminder to insitutions of the variability of these changes. Our results indicate that the simulated changes in claims rates are large and statistically significant over multi-year periods for much of the U.S. Corn Belt. However, we emphasize that these changes do not necessarily exhibit monotonacity and may be very noisy from year to year.
