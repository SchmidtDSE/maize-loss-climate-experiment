...
# Methods
We first build predictive models of maize yield distributions using a neural network at an insurer-relevant spatial scale. We then estimate changes to yield losses under different climate conditions with Monte Carlo simulation in order to estimate crop loss probability and severity. Finally, these changes to yield estimate insurance claims rates.
...
$y_{\Delta\%}(x) = \frac{y_{actual} - y_{expected}}{y_{expected}} = \frac{y_{\Delta}}{y_{\mu-historic}} = f(C_z, y_{\mu-historic-z}, y_{\sigma-historic-z})$
