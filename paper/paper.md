...
### Limitations
Despite this similarity between predictions and recent actuals, a number of difficult to model factors likely lead us to underestimate the actual claims rate. Field-level yield data and the actual geographically specific risk unit structure are not currently public. Therefore, while we sample units randomly based on expected size, growers likely optimize their own unit structure when purchasing policies to optimize financial upside as further explored in supplemental. Similarly, we do not have the geographically specific data required to model trend adjustment [@plastina_trend-adjusted_2014] and yield exclusion options [@schnitkey_yield_2015].
