...
Due to the size of the input dataset, we cannot take all SCYM data per neighborhood into Monte Carlo. Separately, we must avoid dramatic expansions in the output vector size as this could cause the input dataset requirements to exceed feasibility [@alwosheel_dataset_2018]. Therefore, we down-sample these yields to generate yield delta distributions and make distribution shape assumptions so that, instead of predicting full distributions, the neural network can predict distribution shape parameters.

### Yield delta distributions
In generating the historic yield delta distributions ahead of training neural networks, we sample 1000 yield values per neighborhood to represent a growing year and then separately sample to represent historic averages. This avoids needing to make distributional assumptions about yield ahead of neural network operation.

### Distribution shape assumption
The neural network does require a distributional shape assumption which we decide based on observed skew and kurtosis of yield deltas. Our open source pipeline can be run with a trivial configuraiton change where one may force neighborhood yield deltas to fit beta or normal distributions. The former has precedent in the literature [@nelson_influence_1990]. Even so, we observe that most^[Around 97% - 99% of neighborhoods and maize growing acreage depending on yield downsampling are approximately normal per @kim_statistical_2013.] of yield delta distributions exhibit approximate normality [@kim_statistical_2013]. While leaving the beta distributions in place regardless of this finding may offer further flexibility, using beta distributions in our neural networks results in similar median absolute errors but elevated mean absolute errors as shown in Table . 



Further investigation finds that that a minority population of neighborhoods causes this swing where small changes in beta distribution parameters can infrequently cause large error. As prediction of that population shows stronger performance under a normality assumption for yield deltas, we prefer this approach in our main text. For consistency, all neighborhoods are predicted using a normal distribution assumption for yield deltas.
...
## Grower behaviors
Some grower behaviors are difficult to capture within our modeling.
